...
async function migrateProducts(barData: RawBarData[], supplierNameMap: Map<string, string>) {
    ...
    for (const product of barData) {
        ...
        batch.set(docRef, productData, { merge: true }); // Use merge to prevent overwriting existing data if re-run
        newProductsCount++;
        operations++;
...
`);
    ...
    try {
        ...
        // --- Run Migrations ---
        // NOTE: Inventory is NOT cleared by default anymore to allow for multiple runs.
        // If you need a completely clean import, uncomment the next line.
        // await clearCollection('inventory');
        
        await migrateSuppliers(supData);
        await migrateProducts(barData, supplierNameMap);
        
        // Clearing inventory before migration to ensure a fresh import each time
        await clearCollection('inventory');
