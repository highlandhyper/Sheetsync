...
import { PlusCircle, Loader2, Search, Save, Check, ChevronsUpDown, DollarSign, Edit, Zap } from 'lucide-react';
...
export function EditOrCreateProductForm({ allSuppliers }: EditOrCreateProductFormProps) {
  ...
  const handleMagicLookup = async () => {
    ...
    try {
        ...
        if (res.success && res.data) {
            ...
            toast({ title: "Magic Lookup Success", description: `Retrieved product identity.` });
        } else {
            toast({ title: "No Result", description: "Not found in global registry.", variant: "destructive" });
        }
    } catch (e) {
        toast({ title: "Error", description: "Global database unreachable.", variant: "destructive" });
    } finally {
  ...
  const handleSearchBarcode = async (barcode?: string) => {
    ...
    if (!barcodeToUse) {
      toast({ title: 'Barcode Required', variant: 'destructive' });
      return;
    }
  ...
  const processFormSubmit = (data: AddProductFormValues) => {
    ...
    formData.append('costPrice', data.costPrice === undefined ? '' : String(data.costPrice));
    ...
    toast({ title: 'Update Applied', description: 'Syncing with registry...' });
    ...
    startSaveTransition(async () => {
      try {
        ...
        if (result.success) refreshData();
            ...
            toast({ title: 'Sync Error', description: result.message || 'Registry sync failed.', variant: 'destructive' });
            refreshData(); 
        }
      } catch (e) {
          refreshData();
  ...
  const handleEditSupplierClick = () => {
    ...
    if (selectedSupplier) {
      ...
      toast({ title: "Selection Error", description: "Select a registered vendor.", variant: "destructive" });
    }
  };
    ...
    <Card className="w-full max-w-4xl mx-auto shadow-xl border-primary/10 overflow-hidden">
      ...
      <CardContent className="space-y-6 pt-6">
        <div className="space-y-3">
          <Label className="text-xs font-black uppercase text-muted-foreground tracking-widest">Identify Barcode</Label>
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            />
            <Button onClick={() => handleSearchBarcode()} disabled={isFetchPending || !barcodeToSearch.trim()} className="w-full sm:w-auto h-12 font-black uppercase tracking-tighter">
          ...
          <form onSubmit={handleSubmit(processFormSubmit)} className="space-y-6 border-t pt-8 mt-4 animate-in slide-in-from-top-4 duration-500">
              {productNotFound ? (
                <Alert className="bg-primary/5 border-primary/20 rounded-3xl relative overflow-hidden p-6">
                    <div className="absolute top-4 right-4">
                        <Button type="button" variant="outline" size="sm" onClick={handleMagicLookup} disabled={isMagicLoading} className="h-9 text-[10px] font-black uppercase bg-white/80 border-primary/30 shadow-sm">
                    ...
                    <AlertDescription className="text-xs font-medium text-muted-foreground">SKU <span className="font-mono font-black text-primary bg-primary/10 px-1.5 py-0.5 rounded">{searchedBarcode}</span> is new.</AlertDescription>
                </Alert>
              ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Barcode (Key)</Label>
                ...
                <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Identification Name</Label>
                    <Input
                        id="productName"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                <div className="space-y-2">
                    <div className="flex items-center justify-between mb-1">
                        ...
                        <Button type="button" variant="ghost" size="sm" onClick={handleEditSupplierClick} disabled={!supplierNameValue || !allSuppliers.some(s => s.name.toLowerCase() === supplierNameValue.toLowerCase())} className="text-[9px] uppercase font-black h-6 px-2 text-primary">
                    ...
                    <Popover open={supplierComboboxOpen} onOpenChange={setSupplierComboboxOpen}>
                      ...
                      <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
                        <Command>
                          <CommandInput placeholder="Search or type new..." value={supplierSearchTerm} onValueChange={setSupplierSearchTerm} onKeyDown={(e) => { if (e.key === 'Enter' && supplierSearchTerm) { setValue('supplierName', supplierSearchTerm, { shouldValidate: true }); setSupplierComboboxOpen(false); setTimeout(() => costInputRef.current?.focus(), 100); } }} />
                          <CommandList>
                            <CommandEmpty>
                                {supplierSearchTerm ? (
                                    <Button variant="ghost" className="w-full justify-start text-xs h-9 font-black uppercase" onClick={() => { setValue('supplierName', supplierSearchTerm, { shouldValidate: true }); setSupplierComboboxOpen(false); setTimeout(() => costInputRef.current?.focus(), 100); }}>
                            ...
                            <CommandGroup>
                              {sortedSuppliers.map((supplier) => (
                                <CommandItem key={supplier.id} value={supplier.name} onSelect={() => { setValue("supplierName", supplier.name, { shouldValidate: true }); setSupplierComboboxOpen(false); setTimeout(() => costInputRef.current?.focus(), 100); }} className="font-bold text-xs">
                ...
                <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Cost Valuation (QAR)</Label>
              ...
              <div className="pt-6 flex justify-end">
                <Button type="submit" disabled={isSavePending} className="w-full sm:w-auto h-12 px-10 font-black uppercase tracking-tighter shadow-xl shadow-primary/20 rounded-2xl">
                  {isSavePending ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <Save className="mr-2 h-5 w-5" />}
